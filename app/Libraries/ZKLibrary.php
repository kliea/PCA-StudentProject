...
class ZKLibrary
{
    ...
    private function decodeTime($data)
    {
        // CHANGES: had to add floor function to avoid implicit converstion error, this shoudln't change thigns, probably
        $second = $data % 60;
        $data = floor($data / 60); 
        $minute = $data % 60;
        $data = floor($data / 60); 
    ...
    public function createHeader($command, $chksum, $session_id, $reply_id, $command_string)
    {
        ...
        if (is_array($u)) {
            foreach ($u as $key => $value){ // BAYLO : each() rani diri sauna pero di na daw pwede na sa php 8
                $u = $u[$key];
                break;
            }
        }
    ...
    public function execCommand($command, $command_string = '', $offset_data = 8)
    {
        ...
        $reply_id = hexdec($u['h8'] . $u['h7']);
        $buf = $this->createHeader($command, $chksum, $session_id, $reply_id, $command_string);
        socket_sendto($this->socket, $buf, strlen($buf), 0, $this->ip, $this->port); // BAYLO : akong gi bayloan ang MSG_EOR to 0, basin importante puhon
        try {
            socket_recvfrom($this->socket, $this->received_data, 1024, 0, $this->ip, $this->port);
            $u = unpack('H2h1/H2h2/H2h3/H2h4/H2h5/H2h6', substr($this->received_data, 0, 8));
            $this->session_id =  hexdec($u['h6'] . $u['h5']);
        ...
        try {
            ...
            if (count($this->attendance_data) > 0) {
                ...
                while (strlen($attendance_data) > 40) {
                    ...
                    $state = hexdec(substr($u[1], 66, 2)); // Changes: from 56 to 66. i got the solution from the github repo
