...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // SSL ===================================================================
        $salaryGrades = [];
        $uniqueValues = range(10000, 200000); // Create a range of unique values
        shuffle($uniqueValues); // Shuffle to randomize the order
        $currentIndex = 0; // Track the index for assigning unique values
        
        for ($grade = 1; $grade <= 33; $grade++) {
            $steps = [];
            for ($step = 1; $step <= 8; $step++) {
                if ($grade < 33 || $step <= 2) {
                    $steps["step{$step}"] = $uniqueValues[$currentIndex];
                    $currentIndex++;
                } else {
                    $steps["step{$step}"] = 0;
                }
